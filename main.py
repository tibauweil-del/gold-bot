...
def detecter_liquidite_pro(df, tolerance=0.0010):
    """Détecte uniquement les zones majeures (3 touches min)"""
    ...
    for i in range(len(df) - 100, len(df)):
        ...
        if np.sum(np.abs(highs - p_h) < (p_h * tolerance)) >= 3:
            if not any(abs(z - p_h) < 15 for z in bsl_zones): bsl_zones.append(p_h)
        if np.sum(np.abs(lows - p_l) < (p_l * tolerance)) >= 3:
            if not any(abs(z - p_l) < 15 for z in ssl_zones): ssl_zones.append(p_l)
...
def moteur_algo_smc_pro():
    ...
    # --- FILTRE VOLUME (x1.5) ---
    if volume_actuel < (volume_moyen * 1.5): return 
...
def formater_alerte(direction, zone, prix, vol, tp, sl):
    """Garantit que la structure des messages est identique pour Long et Short"""
...
def envoi_rapport_final(bsl, ssl, prix, df):
    ...
    msg = (f"🏛️ ANALYSE POST-SESSION US\n"
           ...
           f"• SSL (Achat attendu) : {sorted(ssl)[-1] if ssl else 'N/A':.2f}$\n"
           f"• BSL (Vente attendue) : {sorted(bsl)[0] if bsl else 'N/A':.2f}$\n"
           f"----------------------------\n"
